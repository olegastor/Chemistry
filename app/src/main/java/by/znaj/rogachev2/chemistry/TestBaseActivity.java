...
public class TestBaseActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        cursorBases = db.rawQuery("select * from " + DatabaseHelper.TABLE_BASE + " ORDER BY RANDOM() LIMIT 10", null);

        if (cursorBases.getCount() != 0 && cursorBases.getCount() == 10) {
            cursorBases.moveToFirst();
            nextQuestion();
        } else {
    ...
    public void nextQuestion() {
        ...
        textQuestion.setText(makeStringLikeFormula(cursorBases.getString(1)), TextView.BufferType.SPANNABLE);
        baseId = cursorBases.getLong(0);

        cursorAnswers = db.rawQuery("select * from " + DatabaseHelper.TABLE_BASE_ANSWERS + " where " + DatabaseHelper.COLUMN_ID_BASE + "=? ORDER BY RANDOM()", new String[]{String.valueOf(baseId)});
